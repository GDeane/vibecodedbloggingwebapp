import prisma from "./prisma";
import { Prisma } from "@prisma/client";

export type FeedPost = {
  id: number;
  content: string;
  createdAt: Date;
  author: { id: number; username: string; displayName?: string | null };
};

// cursor is a string in format `${createdAt.toISOString()}::${id}`
export async function getGlobalFeed({ cursor, limit = 5 }: { cursor?: string; limit?: number }) {
  let cursorDate: Date | undefined;
  let cursorId: number | undefined;

  if (cursor) {
    const parts = cursor.split("::");
    if (parts.length === 2) {
      cursorDate = new Date(parts[0]);
      cursorId = parseInt(parts[1], 10);
      if (Number.isNaN(cursorId)) cursorId = undefined;
    }
  }

  const where: Prisma.PostWhereInput = { parentPostId: null };

  if (cursorDate) {
    where.OR = [
      { createdAt: { lt: cursorDate } },
      { AND: [{ createdAt: cursorDate }, { id: { lt: cursorId ?? 0 } }] },
    ];
  }

  const take = (limit ?? 5) + 1; // fetch one extra to detect next page

  const rows = await prisma.post.findMany({
    where,
    include: { author: true },
    orderBy: [{ createdAt: "desc" }, { id: "desc" }],
    take,
  });

  let nextCursor: string | null = null;
  const posts = rows.slice(0, limit ?? 5).map((p) => ({
    id: p.id,
    content: p.content,
    createdAt: p.createdAt,
    author: { id: p.author.id, username: p.author.username, displayName: p.author.displayName },
  })) as FeedPost[];

  if (rows.length === take) {
    const last = rows[rows.length - 1];
    nextCursor = `${last.createdAt.toISOString()}::${last.id}`;
  }

  return { posts, nextCursor };
}
